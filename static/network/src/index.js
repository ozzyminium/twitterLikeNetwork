...
// edit and delete.
function ThreeDotsSelect({ post, handleDelete }) {
    const username = document.querySelector("#root").dataset.user;
...
function ThreeDotsDropdown({posts, setPosts, postCount, setPostCount,  handleDelete, post}) {
    const dotRef = useRef();
...
function EditModal({posts, setPosts, postCount, setPostCount, post}) {
    const [textValue, setTextValue] = useState(post.text)
    const updatedPost={text: textValue};
...
function PostContentHeader({name, username, postId, timestamp, posts, setPosts, postCount, setPostCount, post}) {
    const csrftoken = getCookie('csrftoken');
    ...
    const handleDelete = () => {
        fetch(request, {
            ...
            .then(response => {
                if (response.ok){
                    setPosts(posts => posts.filter(x => x.id !==postId));
...
function PostContentFooter({ favCount, postId, favStat }) {
    ...
    const [favNumber, setFavNumber] = useState(favCount);
    const [isFaved, setIsFaved] = useState(favStat === "true" ? true : false);
    const favRef = useRef();
    ...
    const handleLikePost = (e) => {
        if (!isAuth) {
            ...
            fetch(request, {
                ...
                .then(response => {
                    if (response.ok) {
                        setFavNumber(favNumber => favNumber+1);
                        setHeartIcon(heartFill)
    ...
    return(
        <div onClick={isFaved ? handleUnlikePost : handleLikePost} 
            ref={favRef} 
...
function PostContainer({ post, posts, setPosts, postCount, setPostCount }) {
    const { 
        id, 
        is_faved, 
        likeNumber, 
...
// Header to display information about center content
function AllPostsCenterRoof({ postCount }) {
    ...
    let postCountContent = `posts`
    if (postCount) {
         postCountContent = `${postCount} posts`
    }
    return(
        <div className="d-flex  mt-2 mb-2">
            <div className="d-flex align-items-center ps-4 pe-2">
                {hashIcon}
            </div>
            <div className="d-flex">
                <div className="d-flex align-items-end fs-4 fw-bold">
                    <span>Discover</span>
                </div>
                <div className="post-count-container ps-3 pe-3">
                    <span>·</span>
                </div>
                <div className="post-count-container">
                    <span>{postCountContent}</span>
                </div>
            </div>
        </div>
    );
...
// Fetching and rendering all posts for #Discover section.
function DiscoverAllPosts({ posts, setPosts, postCount, setPostCount }) {
    const pageNum = 1;
    const page = {page: pageNum};
    const allPosts = [];
    const handleFetchPosts = () => {
        fetch(`users/posts?` + new URLSearchParams({page: pageNum}), {
            method: "GET",
            mode: "same-origin",
        })
...
function MiniDotsSelect({ auth, user }){
    if(auth === "false") {
        return(
            <React.Fragment>
                <li>
...
// Component to contain center elements.
...
function NavbarButton({ icon, name, user }) {
    let link = "";
    if(name==="Home") {
        link = "home";
    }
...
function NavbarAuth({hashIcon, homeIcon, profileIcon, user, logoutIcon}) {
    return(
        <React.Fragment>
            <AppIcon />
            <NavbarButton icon={homeIcon} name="Home" user={user}/>
            <NavbarButton icon={profileIcon} name="Profile" user={user}/>
            <NavbarButton icon={logoutIcon} name="Log Out" user={user}/>
        </React.Fragment>
    );
};
...
function NavbarSelect(props) {
    const hashIcon = props.hashIcon;
    const loginIcon = props.loginIcon;
    const homeIcon = props.homeIcon;
    const profileIcon = props.profileIcon;
    const logoutIcon = props.logoutIcon;
...
function SideContentSelect() { 
    ...
    if (document.querySelector("#root").dataset.user) {
        return(
            <SideContentAuth />
        );
    }
